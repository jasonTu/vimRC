...
class Powerline:
    ...
    def draw_with_newline(self):
        return (''.join(self.draw_segment(i) for i in range(len(self.segments) - 1))
                + self.reset).encode('utf-8') + ' \n' + (self.draw_segment(len(self.segments) - 1) + self.reset).encode('utf-8') + ' '

    def draw_segment(self, idx):
        segment = self.segments[idx]
        next_segment = self.segments[idx + 1] if idx < len(self.segments)-2 else None
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('--cwd-max-depth', action='store', type=int,
            default=25, help='Maximum number of directories to show in path')
...
sys.stdout.write(powerline.draw_with_newline())
